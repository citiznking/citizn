...
export default {
  fetch: withSupabase({ auth: "publishable" }, async (req, ctx) => {
    ...
    const { election_slug, pu_id, lat, lng, accuracy_m, captured_at, session_uuid } = body;
    ...
    // Arrival check-ins auto-publish (low risk, high corroboration value) —
    // spec: "arrival check-ins and result uploads publish automatically
    // with a corroboration badge".
    ...
    await admin.rpc("compute_checkin_corroboration", { p_pu_id: pu_id });

    return Response.json({ election_report_id: inserted.id, status: "published" }, { status: 201 });
  }),
};
